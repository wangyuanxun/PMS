...
namespace CacheSrvLib.DAL
{
    ...
    {
        #region 系统自动生成服务接口
        const string _fields = "ID,ButtonName,ButtonFunction,ButtonImg,Intro,OrderIndex,CreateTime";
        /// <summary>
        /// 获取表主键最大ID
        /// </summary>
        /// <returns></returns>
        ...
        {
            var obj = SqlHelper.GetObject("SELECT MAX(ID) FROM [Sys_Button] (NOLOCK)");
            if (obj != null)
        ...
        {
            string strSQL = "SELECT COUNT(1) FROM [Sys_Button] (NOLOCK) WHERE ID=@ID";
        ...
        {
            string strSQL = "SELECT COUNT(1) FROM [Sys_Button] (NOLOCK) WHERE " + @selectWhere.FilterSQL();
        ...
        public int GetCount(string selectWhere)
        {
            ...
            string strSQL = "SELECT COUNT(1) FROM [Sys_Button] (NOLOCK) " + @selectWhere.FilterSQL();
        ...
        public Sys_Button GetModel(long ID, string fields = "")
        {
            ...
            string strSQL = "SELECT " + fields.FilterSQL() + " FROM [Sys_Button](NOLOCK) WHERE ID=@ID";
            return SqlHelper.GetEntity<Sys_Button>(strSQL, new[] { new SqlParameter("@ID", ID) });
        }
        ...
        {
            if (fields == "") fields = _fields;
            string strSQL = "SELECT " + fields.FilterSQL() + " FROM [Sys_Button](NOLOCK) WHERE " + @selectWhere.FilterSQL();
            return SqlHelper.GetEntity<Sys_Button>(strSQL);
        }
        ...
        public List<Sys_Button> GetModelList(string fields = "", string topWhere = "", string selectWhere = "", string sort = "")
        {
            ...
            string strSQL = "SELECT " + topWhere.FilterSQL() + " " + fields.FilterSQL() + " FROM [Sys_Button](NOLOCK) " + selectWhere.FilterSQL() + " " + sort.FilterSQL();
            return SqlHelper.GetEntities<Sys_Button>(strSQL);
        }
        ...
        public Page<Sys_Button> GetPageList(int pageIndex, int pageSize, string selectWhere = "", string fields = "", string sort = "")
        {
            ...
            string strSQL = "SELECT " + fields.FilterSQL() + " FROM [Sys_Button](NOLOCK) " + selectWhere.FilterSQL();
            strSQL = SqlHelper.GetPagingSql(strSQL, sort.FilterSQL());
            string sqlCount = @"SELECT COUNT(1) FROM [Sys_Button](NOLOCK) " + selectWhere.FilterSQL();
            return SqlHelper.GetPage<Sys_Button>(pageIndex, pageSize, strSQL, null, sqlCount, null);
        }
        #endregion
